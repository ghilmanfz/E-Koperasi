...
$pending_pinjaman = mysqli_fetch_row(mysqli_query($konek,"SELECT COUNT(*) FROM tbl_pinjaman"))[0] ?? 0;
...
<div class="sk-hero"<?php if (!empty($s['foto_hero'])): ?> style="background-image: linear-gradient(135deg, rgba(30,64,175,0.88) 0%, rgba(99,102,241,0.80) 100%), url('../<?php echo htmlspecialchars($s['foto_hero']); ?>'); background-size: cover; background-position: center;"<?php endif; ?>>
  <div class="sk-hero-tag">Selamat Datang, Sekretaris</div>
...
<div class="sk-stat-grid">
  ...
  <div class="sk-stat-card">
    <div>
      ...
      <div class="sk-stat-value"><?php echo number_format($pending_pinjaman); ?></div>
...
<div class="sk-cta">
  <div class="sk-cta-text">
    <h4><?php echo htmlspecialchars($s['cta_judul']); ?></h4>
    <p><?php echo htmlspecialchars($s['cta_deskripsi']); ?></p>
  </div>
  <a href="dataanggota.php" class="sk-btn sk-btn-primary"><i class="fa fa-users"></i> Lihat Data Anggota</a>
</div>
